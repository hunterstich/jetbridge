...
with agentic coding tools like opencode or gemini-cli from the IDE editor. Inspired by 
...
|----------------------|-----------------------------------------------------|----------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| @<provider>:<target> | @gem:1 How does @this work?                         | (Run in Gemini-CLI with session at index 1) How does @src/Main.kt L5 work? | @<provider>:<target> specifies which provider the prompt should be sent to. Jetbridge provides autocompletion for finding all available providers and targets directly in the prompt dialog as you type |
...
* Trigger the Jetbridge.PromptAction and specify the opencode session you'd like the prompt sent
to with `@oc:<session>`. If a target isn't specified in your prompt, jetbridge will send the prompt
to the most recently used target.

### Gemini-CLI
(Basic implementation available)
* Start gemini-cli in a tmux session with `tmux new-session -s jetbridge 'gemini' (gemini-cli
doesn't have a supported way of controlling the TUI and it's being done by sending 
tmux events)
* Trigger the Jetbridge.PromptAction and specify the tmux session you'd like the prompt sent to with
`@gem:<session>`. If a target isn't specified in your prompt, jetbridge will send the prompt to the
